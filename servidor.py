...
def init_db():
    ...
    with sqlite3.connect(DATABASE) as db:
        ...
        columnas_usuario = {
            columna[1]
            for columna in db.execute(
                "PRAGMA table_info(usuarios)"
            ).fetchall()
        }

        if "salt" not in columnas_usuario:
            db.execute(
...
def authenticate(db, username, password):
    ...
    row = db.execute(
        """
        SELECT id, salt, contrasena_hash
        FROM usuarios
        WHERE usuario = ?
        """,
        (username,)
...
def create_task(request_data):
    ...
    with db_lock, sqlite3.connect(DATABASE) as db:
        ...
        cursor = db.execute(
            """
            INSERT INTO tareas
            (usuario_id, descripcion, creada_en)
            VALUES (?, ?, ?)
            """,
            (
                user_id,
...
def delete_task(request_data):
    ...
    if cursor.rowcount == 0:
        return error(
            "Tarea no encontrada o no te pertenece."
        )

    return success(
        mensaje=f"Tarea #{task_id} eliminada."
    )
...
def handle_client(connection, address, worker_pool):
    ...
    print(
        f"Cliente conectado: {address[0]}:{address[1]}"
    )
    ...
    with connection:
        ...
        for raw_line in file:
            ...
            try:
                ...
                response = error(
                    "El mensaje recibido no es JSON valido."
                )
    ...
    print(
        f"Cliente desconectado: {address[0]}:{address[1]}"
    )
...
def run_server(host, port, max_workers):
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        with socket.socket(
            ...
            print(
                f"Pool de workers activo con {max_workers} hilos"
            )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Servidor TCP para gestion distribuida de tareas."
        )
    )
